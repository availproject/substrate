...
use super::*;
use crate as tips;
use frame_support::{assert_noop, assert_ok, parameter_types, traits::Contains, weights::Weight};
use sp_core::H256;
...
fn tip_new_cannot_be_used_twice() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(TipsModTestInst::tip_new(
            Origin::signed(10),
...
fn report_awesome_and_tip_works() {
    new_test_ext().execute_with(|| {
        Balances::make_free_balance_be(&Treasury::account_id(), 101);
        assert_ok!(TipsModTestInst::report_awesome(
            Origin::signed(0),
            b"awesome.dot".to_vec(),
            3
        ));
        ...
        assert_noop!(
            TipsModTestInst::tip(Origin::signed(9), h.clone(), 10),
            BadOrigin
        );
...
fn close_tip_works() {
    new_test_ext().execute_with(|| {
        ...
        System::set_block_number(2);
        assert_noop!(
            TipsModTestInst::close_tip(Origin::none(), h.into()),
            BadOrigin
        );
